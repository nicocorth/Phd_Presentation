...
\onslide<4->{
\begin{framed}
\textbf{Développement de dispositifs expérimentaux reproduisant les plasmas de réentrée atmosphérique.}
\end{framed}
}
\end{frame}
...
\begin{minipage}[t]{0.5\linewidth}
    ...
    Basé sur le principe de \textbf{transfer de chaleur local} (video).
    }
    %\only<2>
    %{
...
\end{framed}
\includegraphics[width=.7\linewidth]{lhtsT.pdf}\footnote{Courtesy of Thierry Magin.}
\end{center}
\end{frame}
...
\begin{minipage}[t]{0.5\linewidth}
    ...
    $T ~ \SI{10 0000}{K}$, $p \in [\SI{1000}{\pascal}; \SI{100000}{\pascal}]$.
...
\begin{frame}{ICP : what is missing?}
...
\begin{framed}
\textbf{Can we offer a framework able to support future ICP simulations requirements?}
\end{framed}
...
\begin{center}
\begin{tikzpicture}
\node at (0,0) {\includegraphics[width=.65\linewidth]{lhtsT.pdf}};
\node at (0,-.7) {\reflectbox{\includegraphics[width = .65\linewidth, angle=180, origin=c]{./thierry_mesh.png}}};
\end{tikzpicture}
\end{center}
...
\begin{framed}
\textbf{Developing a discontinuous Galerkin solver of inductively coupled plasmas.}
\end{framed}
...
\flushright
\includegraphics[width=0.7\linewidth]{./mpp-logo.png}
\end{minipage}
}
