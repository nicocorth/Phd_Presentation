...
\begin{frame}[c, noframenumbering, plain]
  ...
  \Large
  \centering
  \textbf{A Monolithic High-Order Multi-Domain Solver for Inductively Coupled Plasma}
...
\begin{frame}[c]{Au fait, qu'est-ce que j'ai fait pendant 4 ans?}

\begin{framed}
\centering
J'ai développé un \textbf{solver numérique} pour les \textbf{plasmas} à \textbf{induction}.
\end{framed}
\centering
\includegraphics[height =1.5 cm]{2560px-University_of_Liège_logo.png}\hfill\includegraphics[height=1.5 cm]{FRS-FNRS_ros_vert_transp.png}\hfill\includegraphics[height =1.5 cm]{vki_logo_blue_rectangular.jpg}
\end{frame}
...
\includegraphics[height=.17\linewidth]{ice_cubes.jpg} & \includegraphics[height=.17\linewidth]{eau_liquide.jpg} & \includegraphics[height=.17\linewidth]{bouilloire.jpg}\\
\includegraphics[height=.17\linewidth]{eau_solide.png} & \includegraphics[height=.17\linewidth]{eau_liquide.png} & \includegraphics[height=.17\linewidth]{eau_gaz.png}%\footnote{\tiny\url{https://www.assistancescolaire.com/eleve/3e/physique-chimie/reviser-une-notion/les-\%20etatsde-la-matiere-et-les-changements-d-etat-3_pc_01/print?print=1&printSheet=1}}\\
%\onslide<3->{
...
\centering
\begin{center}
\includegraphics[height=.29\linewidth]{./sun.jpg}
\includegraphics[height=.29\linewidth]{./aurora_borealis.jpeg}
\begin{tikzpicture}
\node at (0,0) {\includegraphics[height=.29\linewidth]{eau_gaz.png}};
...
\end{tikzpicture}\footnote{Courtesy of Wikipedia.}
\end{center}


%\begin{framed}
%\textbf{Un plasma est un gaz quasi-neutre composé de particules chargées 
%%(ions $\textcolor{blue}{\bullet^+}$ et electrons $\textcolor{red}{\bullet^{e^-}}$)
%et neutres 
%%($\bullet^n$)
%démontrant un comportement collectif.\footnotemark}
...
%\footnotetext[1]{\tiny F. F. Chen. Introduction to Plasma Physics and Controlled Fusion. Ed. by Springer International Publisher. 2016}
...
%\includegraphics[height=.22\linewidth]{./lightning.jpg}\footnote{Courtesy of Wikipedia.}
%\end{center}
%Fusion nucléaire, médecine, métallurgie, lasers, création de microprocesseurs, ...
%\end{frame}
...
\centering
\includegraphics[height=.35\linewidth]{./reentry_esa.jpg}\footnote{Coutesy of NASA.}
\begin{framed}
Vitesse de réentrée $\simeq\SI{10}{\kilo\meter\;\second^{-1}}$ : choc transformant l'air en plasma.
\end{framed}
\end{frame}
...
\textbf{TPS} & \textbf{Déchets spatiaux}
\end{tabular}
\begin{framed}
\textbf{Expériences reproduisant les plasmas de réentrée atmosphérique.}
\end{framed}
\end{frame}
...
\begin{minipage}[t]{0.5\linewidth}
    ...
    \begin{tikzpicture}[scale=0.7]
      ...
      \draw (6,1.2) node{\tiny \textcolor{red}{Ecoulement d'air}};
...
\begin{frame}{Plasma à induction: solveurs numériques}

\begin{center}
%\begin{framed}
%\textbf{Des solvers numériques ont été développés afin de préparer au mieux les expériences dans les torches à induction.}
%\end{framed}
\includegraphics[width=\linewidth]{lhtsT.pdf}\footnote{Courtesy of Thierry Magin.}
\end{center}


\begin{framed}
\centering
\textbf{On propose un nouveau type de solver numérique pour les plasmas à induction.}
\end{framed}
...
%\begin{center}
%\begin{framed}
%\textbf{On propose un nouveau type de solver numérique pour les plasmas afin de représenter plus de situations physiques.}
%\end{framed}
%
%\includegraphics[height = .15\linewidth]{./coil_effect.png}
%\includegraphics[height = .15\linewidth]{./plasmatron_nozzle_operating.png}
%\includegraphics[height = .15\linewidth]{./plasmatron_nozzles.png}
...
\begin{frame}{ICP facility: an overview}
\begin{minipage}[t]{0.5\linewidth}
    \centering
    \vspace{0.7cm}
    \begin{tikzpicture}[scale=0.7]
      ...
      \draw (6,1.2) node{\tiny \textcolor{red}{Streamlines}};
      \draw (3,2.3) node{\tiny \textcolor{black}{Coil}};
      \draw (-1,0) node{\tiny \rotatebox{90}{\textcolor{blue}{Inflow}}};
      \end{scope}
    ...
    \begin{itemize}
    \item $T \simeq \SI{10 000}{K}$, $p \in [\SI{1000}{\pascal}; \SI{100000}{\pascal}]$.
    \item $Re \simeq 100$, $Ma \simeq 0.01$.
    \item Air, argon, carbon dioxide. 
    \end{itemize}
    \end{minipage}
    \begin{minipage}[t]{0.39\linewidth}
    \begin{itemize}
    \item At LTE \& non equilibrium.
    \item Local heat transfer principle. 
    \end{itemize}
    \end{minipage}
    
\end{frame}

\begin{frame}{ICP : geometry}
\begin{center}
...
\begin{frame}{ICP simulations : complex physics at play}

\begin{center}
\includegraphics[height = .2\linewidth]{./coil_effect.png}
\includegraphics[height = .2\linewidth]{./plasmatron_nozzle_operating.png}
\includegraphics[height = .2\linewidth]{./plasmatron_nozzles.png}
\includegraphics[height=.2\linewidth]{./plasma_jet_unsteady_experimental.png}
\end{center}

\end{frame}

\begin{frame}{ICP : existing solvers}
...
\item Subsonic.
\item Steady-state.
\item Local thermodynamic equilibrium.
\item No-radiation.
\end{itemize}
\end{minipage}
\begin{minipage}{0.49\linewidth}
\begin{itemize}
\item Relatively slow convergence.
\item Requires a constant attention during convergence processes.
\end{itemize}
\end{minipage}
\end{frame}

\begin{frame}{Finite volume solver limitations.}

\begin{center}
\begin{tikzpicture}
%\node at (0,0) {\includegraphics[width=.9\linewidth]{lhtsT.pdf}};
%\node at (0,-1.) {\reflectbox{\includegraphics[width = .9\linewidth, angle=180, origin=c]{./thierry_mesh.png}}};
\node at (0,0.) {\includegraphics[width = .9\linewidth]{./thierry_mesh.png}};
\end{tikzpicture}
\end{center}

\begin{minipage}{0.49 \linewidth}
\begin{itemize}
\item \textbf{Constant solution on each element: refined mesh large gradient region.}

\item \textbf{Dissipative, "smoothing" the physics.}

\item \textbf{Structured mesh.}
\end{itemize}
\end{minipage}
\begin{minipage}{0.49\linewidth}
\includegraphics[width=\linewidth]{./No_nice_physics.png}
\end{minipage}
...
\begin{framed}
\textbf{Developing a hybridized discontinuous Galerkin solver of inductively coupled plasmas.}
\end{framed}

\begin{itemize}
\item Solution is not constant anymore on every element.
\item Much greater mesh flexibility and (hopefully) less mesh elements.
\item We hope to make a more robust solver.
\item More precise (less dissipative) solver than finite volumes.
\end{itemize}
...
%	\begin{framed}
%	\centering
%	Si l'énergie reçue est suffisamment grande, l'électron est arraché: il devient \textcolor{red}{\textbf{libre}}. L'atome d'hydrogène a été \textbf{\textcolor{red}{ionisé}}.
%	\end{framed}
%}
%
%\end{center}
...
\begin{frame}{Plasma collective behaviour}

\begin{framed}
\textbf{A plasma is a quasi-neutral gas composed of charged 
%(ions $\textcolor{blue}{\bullet^+}$ et electrons $\textcolor{red}{\bullet^{e^-}}$)
and neutral particles 
%($\bullet^n$)
exhibiting a \underline{collective} behaviour (through collisions = particles interaction).}
%A plasma is a quasi-neutral gas of charged and neutral particles which exhibits collective behaviour.
\end{framed}
\centering
\begin{tabular}{cc}
\textbf{Short ranged} & \textbf{Long ranged}\\
\begin{tikzpicture}
...
\end{tikzpicture}\\
Direct contact (local \& binary) & Electric force (distance and collective)
\end{tabular}
\begin{framed}
For scales $>$ Debye length ($~ \SI{1}{\micro\meter}$ in our case), $q \simeq 0$ due to electric force.
\end{framed}
\end{frame}

\begin{frame}{Cold plasmas and thermal equilibrium}

\begin{framed}
\textbf{"Cold plasmas" store energy in the free electrons. It is then transmitted during collisions to the ions and neutrals.}
\end{framed}
%En fonction de l'efficacité des collisions, deux scénarii sont possibles:
\begin{center}
\begin{tabular}{cc}
\textbf{Innefficient collisions} ($T_e \gg T_h$) & \textbf{Efficient collisions} ($T_e \simeq T_h$)\\
\begin{tikzpicture}[scale=3]
\node[blue] at (0,0) {$\bullet$};
\node[red] at (.4,.3) {$\bullet$};
...
\node[red] at (.1,.4) [above right]{$e^-$};
\end{tikzpicture}\\
Thermal non-equilibrium & Thermal equilibrium
\end{tabular}
\end{center}

%\textbf{Plasmas are in thermal equilibrium in this work.}

\end{frame}

\begin{frame}{Chemical equilibrium state}

\begin{framed}
\centering
\textbf{Collisions may lead to chemical reactions.}
\begin{equation*}
N + O \leftrightharpoons NO^+ + e^-
\end{equation*}
\end{framed}

%If $\tau_{chem}$ and $\tau_{hydro}$ are characteristic chemical reaction and flow time respectively:
\begin{center}
\begin{tabular}{ccc}
...
%\textbf{Plasmas are in chemical equilibrium in this work.}

\begin{framed}
\centering
\textbf{Thermal + Chemical equilibrium = Local Thermodynamic Equilibrium (LTE)}
\end{framed}

\end{frame}

\begin{frame}{General equation for unmagnetized, optically thick plasma at LTE}
\begin{framed}
\begin{equation*}
	\left\{
	\begin{aligned}
		& \partial_t \rho + \nabla \cdot (\rho \vecv) &&= 0\\
		&\partial_t (\rho \vecv) + \nabla \cdot \left(\rho \vecv \vecv + p\mathbb{I}\right) - \nabla \cdot \vectau &&= \vecj \times \vecB\\
		&\partial_t \left(\frac{1}{2}\rho ||\vecv||^2 + \rho e\right) + \nabla \cdot \left(\rho H\vecv\right) + \nabla \cdot \left(\vecq - \vectau \cdot \vecv \right) &&= \vecj \cdot \vecE
	\end{aligned}
	\right.
\end{equation*}
\end{framed}
...
\begin{equation*}
\begin{aligned}
& \vectau = \eta \left[\nabla \vecv + \left(\nabla \vecv\right)^T - \frac{2}{3} \left(\nabla \cdot \vecv\right) \mathbb{I}\right] \\
& \vecq = - \lambda \nabla T \\
& \vecj = \sigma \vecE - \sum_{i,j\in S} n_i q_i D_{ij} \left[\frac{\nabla p_j}{n k_B T_h} - \frac{y_j p}{n k_B T_h} \nabla \ln(p) + \left(k_{T}^h + k_{T}^e\right) \nabla \ln(T)\right]
\end{aligned}
\end{equation*}
\end{minipage}
\begin{minipage}[t]{0.29\linewidth}
\flushright
\includegraphics[width=1.\linewidth]{./mpp-logo.png}
\vfill
\end{minipage}

%Unmagnetized plasma and no elemental de-mixing.

\end{frame}

%\begin{frame}{Transport properties : Mutation++}
%
%%\begin{framed}
%%\centering
...
%\centering
%\begin{center}
%\includegraphics[width=.8\linewidth]{./mpp-logo.png}
%\end{center}
%
...
\begin{frame}{Navier-Stokes and Maxwell equations}
\begin{framed}
\begin{equation*}
	\left\{
	\begin{aligned}
...
\begin{equation*}
\begin{aligned}
& \nabla \cdot \vecE = 0 && && \partial_t \vecB + \nabla \times \vecE = \mathbf{0}\\
& \nabla \cdot \vecB = 0 && && -\epsilon_0 \partial_t \vecE + \frac{1}{\mu_0} \nabla \times \vecB = \vecj
\end{aligned}
\end{equation*}
\end{framed}
\end{frame}

\begin{frame}[c]{Hypothesis specific to ICP}
...
\item Quasi-steady \& averaged over an oscillation period.
...
\item No displacement current.

$\varepsilon_0 \partial_t \vecE \simeq 0$

\item Ambipolar electrostatic field ($j_z = j_r = 0$).
...
\item Split of the induced electric field in a coil and plasma part 

$E_I = E_C + E_P$

\item The coils are thin parallels wires surrounding the facility.

$ \sum_{l=1}^{N_{coil}} i f \mu_0 I_C \sqrt{\frac{r_0}{r}}\left[2 \frac{E_2(k_l)}{k_l} - E_1(k_l)\left(\frac{2}{k_l}-k_l\right)\right] $
...
$\mu_0$ : magnetic permeability, $\sigma_e$ : electron electric conductivity, $f$ : induction frequency
\end{framed}
}
...
\begin{frame}{Computational domain and boundary conditions}
...
\begin{tikzpicture}[scale = .6]
	...
	\draw (12,3.8) node[above]{Freestream};
	\draw (15,2) node[above, rotate = -90]{Opening};
	\draw (4,3.8) node[above]{Far electric field};
	\draw (-3,0) node[above, rotate = 90]{Far electric field};
	\draw (0.2,0) node[above, rotate = -90]{Isothermal};
	\draw (4,-1.3) node[above]{No slip isothermal};
	\draw (14, -0.7) node[below]{Isothermal};
	\draw (8.7, -2.55) node[above, rotate = -90] {Coflow};
...
%\end{framed}
%}
For N-S: $\vecw = \left(\rho \quad \rho \vecv \quad \rho e + \rho \frac{||\vecv||^2}{2} \right)$, $\vecu = \left(p \quad \vecv \quad T \right)$
%\textbf{Maxwell}
%\begin{equation*}
%\begin{aligned}
%
...
\begin{frame}{Model problem and weak form}
...
with
\begin{equation*}
(a,b)_{K} = \int_K a b\;dV,\quad \langle a,b \rangle_{\partial K} = \int_{\partial K} a  b \;dS
\end{equation*}}
%\only<3>{
%Discretization of $\Omega$, functional space and solution $\vecu$ + interface numerical fluxes.
%}

\end{frame}

\begin{frame}[c]{Weak form: spatial discretization.}

Division of $\Omega$ into elements ($K$):
...
\begin{equation*}
\sum_K \left(\partial_t \vecw - \vecS, w \right)_{K} - \left(\vecF_{c} - \vecF_{v}, \nabla w \right)_{K} + \left\langle \left(\vecF_{c} - \vecF_{v}\right) \cdot \vecn, w \right\rangle_{\partial K} = 0, \; \forall w \in L_2(\Omega)
\end{equation*}

\end{frame}

\begin{frame}{Weak form: Functional space reduction and solution}
...
If $\{\varphi_{K,i}\}$, $i = \{1, 2, ..., N_p\}$ is a basis (Lagrange, monomial, ...) of $\mathcal{P}_p(K)$, then 

\begin{equation*}
\left(\partial_t \vecw - \vecS, \varphi_{K,i} \right)_{K} - \left(\vecF_{c} - \vecF_{v}, \nabla \varphi_{K,i} \right)_{K} + \left\langle \left(\vecF_{c} - \vecF_{v}\right) \cdot \vecn, \varphi_{K,i} \right\rangle_{\partial K} = 0
\end{equation*}

$\vecu$ is then discretized as
\begin{equation*}
\vecu_h = \sum_K \sum_{i=1}^{N_p} \vecu_{K,i} \varphi_{K,i}
\end{equation*}
...
%\end{framed}
%
%The domain $\Omega$ is split in a collection of elements $K \in \mathcal{T}$.
%
%\begin{center}
%\includegraphics[width=\linewidth]{./Grid_Independence_mesh_probe_cropped.pdf}
%\end{center}
...
%\end{equation*}
%\end{framed}
%We restrict $w$ to a \textbf{finite dimensional} subset of $L_2(\Omega)$.
%
%\begin{equation*}
...
%\begin{framed}
%\begin{equation*}
%\left(\partial_t \vecw - \vecS, w \right)_{\tesselation} - \left(\vecF_{c} - \vecF_{v}, \nabla w \right)_{\tesselation} + \sum_{K \in \tesselation} \left\langle \textcolor{red}{\vecHcal (\vecu, \nabla \vecu)}, w \right\rangle_{\partial K} = 0, \; \forall w \in W_h
%\end{equation*}
%\end{framed}
%
...
%\begin{equation*}
%\begin{aligned}
%& \left(\partial_t \vecw_{h} - \vecS_{h}, \textcolor{red}{\varphi_{K,i}} \right)_{\textcolor{red}{K}} - \left(\vecF_{c, {h}} - \vecF_{v, {h}}, \nabla \textcolor{red}{\varphi_{K,i}} \right)_{\textcolor{red}{K}} + \left\langle \vecHcal(\textcolor{red}{\vecu_h, \nabla \vecu_h}), \textcolor{red}{\varphi_{K,i}} \right\rangle_{\partial K} = 0,\\
%& \forall \textcolor{red}{\varphi_{K,i}} \in W_h
%\end{aligned}
%\end{equation*}
%\end{framed}
%
%If the $\varphi_{K,i}$ is a local basis of $W_h$ on $K$, $i \in \{1, 2, ..., p\}$,
%\begin{equation*}
...
\begin{equation*}
\begin{aligned}
& \left(\partial_t \vecw_{h} - \vecS_{h}, \varphi_{K,i} \right)_{K} - \left(\vecF_{c, {h}} - \vecF_{v, {h}}, \nabla \varphi_{K,i} \right)_{K} + \left\langle \vecHcal (\vecu_h, \nabla \vecu_h), \varphi_{K,i} \right\rangle_{\partial K} = 0
\end{aligned}
\end{equation*}
\centering
...
%	}
    ...
    \fill[smooth, blue!20, opacity=0.5, tension = 0.] (L) .. controls (LM) .. (M) -- (M) .. controls (MN) .. (N) -- (N) .. controls (NL) ..(L);
...
\end{equation*}
\end{minipage}
%\textbf{Usual functional space choice: polynomials of degree at most $p$.}

%$\varphi_{k,i}$ basis can be Lagrange polynomials, monomial, etc...
%}
...
%\begin{equation*}
%\begin{aligned}
%& \left(\partial_t \vecw_{h} - \vecS_{h}, \varphi_{K,i} \right)_{K} - \left(\vecF_{c, {h}} - \vecF_{v, {h}}, \nabla \varphi_{K,i} \right)_{K} + \left\langle \vecHcal(\vecu_h, \textcolor{red}{\vecq_h}), \varphi_{K,i} \right\rangle_{\partial K} = 0, \\
%& \textcolor{red}{\left(\vecq_h, \vectau_{K,i}\right)_{K} - \left(\vecu_h, \nabla \vectau_{K,i}\right)_{K} + \left\langle \vecu_h, \vectau_{K,i} \cdot \vecn \right\rangle_{\partial K_0} + \left\langle \vecu_{bc}, \vectau_{K,i} \cdot \vecn \right\rangle_{\partial K_{bc}} = 0,} \\
%& \forall \varphi_{K,i} \in W_h, \; \textcolor{red}{\forall \vectau_{K,i} \in V_h}
%\end{aligned}
%\end{equation*}
%\end{framed}
%
...
%\begin{equation*}
%\begin{aligned}
%& \left(\partial_t \vecw_{h} - \vecS_{h}, \varphi_{K,i} \right)_{K} - \left(\vecF_{c, {h}} - \vecF_{v, {h}}, \nabla \varphi_{K,i} \right)_{K} + \left\langle \vecHcal(\textcolor{red}{\veclambda_h}, \vecu_h, \vecq_h), \varphi_{K,i} \right\rangle_{\partial K} = 0,\\
%& \left(\vecq_h, \vectau_{K,i}\right)_{K} - \left(\vecu_h, \nabla \vectau_{K,i}\right)_{K} + \left\langle \textcolor{red}{\veclambda_h}, \vectau_{K,i} \cdot \vecn \right\rangle_{\partial K_0} + \left\langle \vecu_{bc}, \vectau_{K,i} \cdot \vecn \right\rangle_{\partial K_{bc}} = 0\\ 
%& \forall \varphi_{K,i} \in W_h, \; \forall \vectau_{K,i} \in V_h
%\end{aligned}
%\end{equation*}
%\end{framed}
%
...
%\end{equation*}
%%and $\veclambda$ the hybrid unknowns defined on the function subset
%\begin{equation*}
%M_h = \left\{\mu \in L^2(\Gamma) : \mu|_e \in \mathcal{P}^p(e), \forall e\in \Gamma\right\}
%\end{equation*}
%%and local basis $\mu_{e,i}$
%\begin{equation*}
%\veclambda_h = \sum_{e\in\Gamma}\sum_{i=1}^{N_p} \veclambda_{e,i} \mu_{e,i}
%\end{equation*}
%
%}
%
...
%\begin{equation*}
%\begin{aligned}
%& \left(\partial_t \vecw_{h} - \vecS_{h}, \varphi_{K,i} \right)_{K} - \left(\vecF_{c, {h}} - \vecF_{v, {h}}, \nabla \varphi_{K,i} \right)_{K} + \left\langle \vecHcal(\veclambda_h, \vecu_h, \vecq_h), \varphi_{K,i} \right\rangle_{\partial K} = 0,\\
%& \left(\vecq_h, \vectau_{K,i}\right)_{K} - \left(\vecu_h, \nabla \vectau_{K,i}\right)_{K} + \left\langle \veclambda_h, \vectau_{K,i} \cdot \vecn \right\rangle_{\partial K_0} + \left\langle \vecu_{bc}, \vectau_{K,i} \cdot \vecn \right\rangle_{\partial K_{bc}} = 0,\\
%& \textcolor{red}{\left\langle[[\vecHcal]], \mu_{e,i} \right\rangle_{e} = 0},\\ 
%& \textcolor{red}{ \forall \mu_{e,i} \in M_h}, \; \forall \varphi_{K,i} \in W_h, \; \forall \vectau_{K,i} \in V_h
%\end{aligned}
%\end{equation*}
%\end{framed}
%
%Equation for $\veclambda$: conservativity of the normal numerical flux across the interfaces.
%
...
%}
\end{frame}
\begin{frame}{Hybridized Discontinuous Galerkin}
%\only<11->
%{
%\begin{framed}
%\small
...
& \left(\partial_t \vecw_{h} - \vecS_{h}, \varphi_{K,i} \right)_{K} - \left(\vecF_{c, {h}} - \vecF_{v, {h}}, \nabla \varphi_{K,i} \right)_{K} + \left\langle \vecHcal(\veclambda_h, \vecu_h, \vecq_h), \varphi_{K,i} \right\rangle_{\partial K} = 0\\
& \left(\vecq_h, \vectau_{K,i}\right)_{K} - \left(\vecu_h, \nabla \vectau_{K,i}\right)_{K} + \left\langle \veclambda_h, \vectau_{K,i} \cdot \vecn \right\rangle_{\partial K_0} + \left\langle \vecu_{bc}, \vectau_{K,i} \cdot \vecn \right\rangle_{\partial K_{bc}} = 0\\
& \left\langle[[\vecHcal]], \mu_{e,i} \right\rangle_{e} = 0
\end{aligned}
\end{equation*}
%\end{framed}
\begin{minipage}{0.69\linewidth}
...
\begin{aligned}
& \vecu_h = \sum_K \sum_{i=1}^{N_p} \vecu_{K,i} \varphi_{K,i}\\
& \vecq_h = \sum_K \sum_{i=1}^{N_p} \vecq_{K,i} \vectau_{K,i} \\
& \veclambda_h = \sum_e \sum_{i=1}^{N_p} \veclambda_{e,i} \mu_{e,i}
\end{aligned}
\end{equation*}
\end{minipage}
%}
...
\textbf{Low mach requires to use preconditioned numerical flux}
\begin{framed}
\begin{equation*}
...
\begin{frame}{Solution strategy: Newton solver with damping term}

\begin{framed}
\begin{equation*}
\mathcal{N}(\vecx) = 0 \Rightarrow \mathcal{N}'(\vecx_k) \delta \vecx_k = - \mathcal{N}(\vecx_k) 
\end{equation*}
\end{framed}
with damping term
\begin{equation*}
...
\begin{equation*}
    ...
    \end{aligned}
\end{equation*}
System solved for $\veclambda$, then $\vecu$ and $\vecq$ are retrieved by small local matrices inversion.
\begin{equation*}
    \begin{pmatrix}
        \delta Q\\
        \delta U\\
...
\textbf{If the power dissipated is not maintained, the torch quenches!}
...
%	\begin{aligned}
%		& \partial_t \rho + \nabla \cdot \left(\rho \vecv\right) =0\\
%		& \partial_t \left(\rho \vecv\right) + \nabla \cdot \left(\rho \vecv \vecv\right) + \nabla p - \nabla \cdot \vectau = \vecF^L\\
%		& \partial_t \left(\rho e + \frac{1}{2} \rho ||\vecv||^2\right) + \nabla \cdot \left(\rho e \vecv + \frac{1}{2} \rho ||\vecv||^2 \vecv + p \vecv - \vectau \cdot \vecv - \vecq\right) = P_J\\
%	\end{aligned}
%\end{equation*}
%
%\begin{equation*}
%	\Delta E_P = \frac{E_P}{r^2} + i 2 \pi f\mu_0\sigma_e \left(E_C + E_P\right)
%\end{equation*}
%}
...
\begin{equation*}
	\begin{aligned}
		& \partial_t \rho + \nabla \cdot \left(\rho \vecv\right) =0\\
		& \partial_t \left(\rho \vecv\right) + \nabla \cdot \left(\rho \vecv \vecv\right) + \nabla p - \nabla \cdot \vectau = \textcolor{red}{\gamma \left(\vecF^L\right)'}\\
		& \partial_t \left(\rho e + \frac{1}{2} \rho ||\vecv||^2\right) + \nabla \cdot \left(\rho e \vecv + \frac{1}{2} \rho ||\vecv||^2 \vecv + p \vecv - \vectau \cdot \vecv - \vecq\right) = \textcolor{red}{\gamma P_J'}\\
		& \Delta \textcolor{red}{E_P'} = \frac{\textcolor{red}{E_P'}}{r^2} + i 2 \pi f\mu_0\sigma_e \left(\textcolor{red}{E_C'} + \textcolor{red}{E_P'}\right) 
	\end{aligned}
\end{equation*}
\end{minipage}
...
E_P = \sqrt{\gamma} E_P'\\
E_C = \sqrt{\gamma} E_C'\\ 
\gamma = \frac{P_{target}}{\int_{NS}P_J' dV}
\end{aligned}
\end{equation*}
\end{minipage}
%}
...
\only<1>
{
\hspace{-1cm}\input{./Convergence_Study_Plot_p_0} &
\input{./Convergence_Study_Plot_u_0}
}
\only<2>
{
\hspace{-1cm}\input{./Convergence_Study_Plot_v_0} & \input{./Convergence_Study_Plot_T_0}
}
\end{tabular}
\only<3>
{
\begin{tabular}{c}
	\input{./Convergence_Study_Plot_utheta_0}
\end{tabular}
}
...
\only<1>
{
\hspace{-1cm}\input{./Convergence_Study_Plot_EpRe_0} & \input{./Convergence_Study_Plot_EpIm_0}
}
\only<2>
{
\hspace{-1cm}\input{./Convergence_Study_Plot_EpRe_1} & \input{./Convergence_Study_Plot_EpIm_1}
}
\end{tabular}
...
\begin{frame}{Mesh and order dependence study}
...
\begin{framed}
\centering
\textbf{This mesh at $p=4$ with 4013 elements for the probe case and 3055 for the freestream case is sufficient for capturing the solution accurately.}
\end{framed}

The mesh size close to the probe is $\SI{10}{\micro\meter}$ thick, while it is of $\SI{1}{\micro\meter}$ for FV!
...
%\only<5>{\textcolor{red}{Short answer is no, BUT...}}
\end{framed}
%\only<2>
%{
...
\centering
	\begin{tikzpicture}[scale = .55]
		...
		\draw[black] (0,0) node{$\bullet$};
		\draw[black] (0,0) node[below left]{$(0,0)$};
		\draw[blue, thick, ->] (8.5,2.75)-- (9.5, 2.75) node[above right]{$U_{blow} = \chi U_{in}$} --(10.5,2.75);
	\end{tikzpicture}
\end{minipage}
\vfill
\begin{equation*}
...
%}
\end{frame}
\begin{frame}{Effect of the coflow on the simulations}
...
\begin{tabular}{cc}
\input{./HS50mm_Radial_1_N=4163_u} & \input{./HS50mm_Radial_1_N=4163_v}
\end{tabular}

\textbf{However, we were not able to obtain any steady state without coflow.}

\end{frame}

\begin{frame}{Comparison with the COOLFluiD solver (radial profile in the torch)}
...
\begin{center}
\begin{tabular}{cc}
\input{./CF_HDG_Comparison_Torch_Center_dp} &
\input{./CF_HDG_Comparison_Torch_Center_u}
\end{tabular}
\end{center}
...
{
\begin{tabular}{cc}
\input{./CF_HDG_Comparison_Torch_Center_v} & \input{./CF_HDG_Comparison_Torch_Center_T}
\end{tabular}
}
...
{
\begin{tabular}{cc}
\input{./CF_HDG_Comparison_Torch_Center_Ep_Re} & \input{./CF_HDG_Comparison_Torch_Center_Ep_Im}
\end{tabular}
}
...
%}
\small
$p_0 = \SI{5000}{\pascal}$, $Q = \SI{16}{\gram\;\second^{-1}}$, $T_{wall} = T_{inlet} = \SI{350}{\kelvin}$, $P = \SI{100}{\kilo\watt}$, $f = \SI{0.37}{\mega\hertz}$, $S = 0$, air (11 species), $\chi_{HDG} = 0.2$.
...
{
\begin{tabular}{cc}
\input{./resL2.tex} & \input{./CFL_history.tex}
\end{tabular}
}
\only<2>
{
\begin{minipage}{0.49\linewidth}
\input{./GMRES_history.tex}
\end{minipage}
\begin{minipage}{0.49\linewidth}
\textbf{Fast and robust solver for ICP, able to produce meaningful results in 20 minutes of time on a single laptop using 8 OMP threads!}

DOFS $\lambda \simeq \SI{400 000}{}$

DOFs $w \simeq \SI{1 000 000}{}$

DOFS $q \simeq \SI{2 000 000}{}$
...
\begin{framed}
\centering
\textbf{The pressure is almost constant everywhere ($p_0 = \SI{10000}{\pascal}$).}
\end{framed}
...
\begin{frame}{Power dissipated in the facility: impact on mass flow rate}

\centering
\begin{tikzpicture}
	\node at (0,0) {\includegraphics[width = \linewidth]{./HS_Streamlines_min.png}};
	\node at (0,-2.4) {\reflectbox{\includegraphics[width = \linewidth, angle=180, origin=c]{./HS_Streamlines_max.png}}};
\end{tikzpicture}

$P_{tot} = \SI{50}{\kilo\watt}$ (top) and $P_{tot} = \SI{200}{\kilo\watt}$ (bottom)

\end{frame}

\begin{frame}{Power dissipated in the facility: impact in the torch}

\centering
\begin{tabular}{cc}
	\hspace{-.7cm}\input{./Torch_Center_P_T.tex} & \input{./Torch_Center_P_P.tex}
\end{tabular}


\end{frame}

\begin{frame}{Power dissipated in the facility: impact along the stagnation line}

\only<1>
{
\centering
	\begin{tabular}{cc}
		\hspace{-1cm}\input{./Stagnation_P_u.tex} &
		\input{./Stagnation_P_gradv_r.tex}
	\end{tabular}
}
...
\begin{tabular}{cc}
	\centering
	\begin{tabular}{cc}
		\hspace{-1cm}\input{./Stagnation_P_gradT_z.tex} &
		\input{./probe_heat_flux.tex}
	\end{tabular}
\end{tabular}
}
...
\item For all of the above, comparison with experiment when possible.
...
\begin{frame}{Comparison with experiment}

\centering
\begin{minipage}{.7\linewidth}
\input{./comparison_experiment}
\end{minipage}
\begin{minipage}{.29\linewidth}
\begin{itemize}
\item $p_0 = \SI{200}{\milli\bar}$
\item $Q = \SI{16}{\gram\;\second^{-1}}$ 
\item $P_{exp.} = \SI{200}{\kilo\watt}$
\item $P_{num.} = \SI{78}{\kilo\watt}$
\item Freestream, no swirl, air.
\end{itemize}
\end{minipage}

\centering
Comparison of the experimental $T(r)$ at $\SI{385}{\milli\meter}$ from the torch exit, from $r = 0$ to $\SI{40}{\milli\meter}$.
...
\begin{frame}{Plasma governing equations : the Boltzmann equation}

\begin{framed}
\begin{equation*}
\mathcal{D}_{i}(f_{i,I}) = \partial_t f_{i,I} + \mathbf{c}_i \cdot \nabla f_{i,I} + \frac{\mathbf{F}_i}{m_i}\cdot \partial_{\mathbf{c}_{i}} f_{i,I} = \mathcal{S}_{i} + \mathcal{C}_{i}
\end{equation*}
\end{framed}
...
\begin{equation*}
\rho = \sum_{i}\int m_i f_i d \vecc_i. \quad \rho \vecv = \sum_i \int m_i \vecc_i f_i d \vecc_i, ... 
\end{equation*}
...
\only<2>
{
If the system is written as \textcolor{red}{$\tilde{\mathcal{N}} = 0$}, then the Newton method solves
\begin{equation*}
\textcolor{red}{\tilde{\mathcal{N}}'(\vecx_k) \delta \vecx = - \tilde{\mathcal{N}}(\vecx_k)}
\end{equation*}
...
\textbf{In order to stabilize the solver, a damping term is added.}
