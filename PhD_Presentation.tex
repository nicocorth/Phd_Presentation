...
\begin{frame}{Hypothesis specific to ICP}
...
%
%	\begin{itemize}
%	\item Electrons return fast to equilibrium when perturbed by ELM waves.
%	
%	\item ELM wavelength $\gg$ plasma length scale.
%	\end{itemize}
%}
%
%\only<4>
...
%	
%	\begin{center}
%	\includegraphics[height=.42\linewidth]{./coil_effect.png}
%	\includegraphics[height = .42\linewidth]{./plasmatron_nozzle_operating.png}
%	\includegraphics[height = .42\linewidth]{./plasmatron_nozzles.png}
%	\end{center}
%
%}
%
...
%{
%
%Thermal + chemical equilibrium = LTE
%
%}
%
%\only<7>
...
%\only<8>
%{
%\includegraphics[width=.8\linewidth]{helicoidal_motion.png}
%}
%
%\only<9>
%{
...
%\only<10>
%{
The electromagnetic field electric field:
...
\begin{framed}
\centering
\textbf{Why HDG over DG?} As will be seen later, HDG allows for static condensation, effectively reducing the number of DOFs when using a Newton solver.
\end{framed}
}
...
\end{framed}
\begin{itemize}
\item $\varphi_{K,i}$ are discontinuous across elements.

\item If $\varphi_{K,i} = 1$ everywhere $\Rightarrow$ finite volume.
...
$\gamma$ is computed at every Newton iteration by equalizing
\begin{equation*}
\gamma = \frac{P_{target}}{\int_{NS}P_J' dV}
\end{equation*}
with $P_{target}$ the desired power dissipated in the facility.

The power was found to remain constant and $\gamma$ was converging towards a value using this method.
